...
- [ ] **Day 2** — SQLite schema (`src/db/schema.sql`) for items, stock, projects, bom, suppliers, reservations, agent_recommendations. Connection helper (`src/db/connection.py`) that creates the db from schema if it doesn't exist.
- [ ] **Day 3** — Synthetic data generator (`src/db/seed_data.py`): ~15 items, 5 projects, 3–4 suppliers per item, realistic contract/supplier notes text.
